...
class VMOps(baseops.BaseOps):
    def __init__(self, wmi, vmutils, volumeops, images, db, os, multiprocessing, uuid):
        super(VMOps, self).__init__(wmi)
    ...
    def spawn(self, context, instance, image_meta, network_info, block_device_info=None):
        ...
        if not (ebs_root):
            ...
            base_vhd_folder = self._os.path.join(FLAGS.instances_path, instance.name)
            if not self._os.path.exists(base_vhd_folder):
                    LOG.debug(_('Creating folder %s '), base_vhd_folder)
                    self._os.mkdir(base_vhd_folder)
            vhdfile = self._os.path.join(base_vhd_folder, instance.name + ".vhd")
            try:
    ...
    def _create_scsi_controller (self,vm_name):
        ...
        scsicontrl.VirtualSystemIdentifiers = ['{' + str(self._uuid.uuid4()) + '}']
        scsiresource = self._vmutils.add_virt_resource(self._conn, scsicontrl, vm)
    ...
    def _create_disk(self, vm_name, vhdfile):
        ...
        new_resources = self._vmutils.add_virt_resource(self._conn, diskdrive, vm)
        ...
        new_resources = self._vmutils.add_virt_resource(self._conn, vhddisk, vm)
        if new_resources is None:
    ...
    def _create_nic(self, vm_name, mac):
        ...
        new_nic_data.VirtualSystemIdentifiers = ['{' + str(self._uuid.uuid4()) + '}']
        #Add the new nic to the vm.
        new_resources = self._vmutils.add_virt_resource(self._conn, new_nic_data, vm)
        if new_resources is None:
    ...
    def destroy(self, instance, network_info=None, cleanup=True):
        ...
        for disk in diskfiles:
            vhdfile = self._conn_cimv2.CIM_DataFile(Name=disk)
            for vf in vhdfile:
                vf.Delete()
    ...
    def _set_vm_state(self, vm_name, req_state):
        ...
        if len(vms) == 0:
            return False
        (job, ret_val) = vms[0].RequestStateChange(constants.REQ_POWER_STATE[req_state])
        success = False
        if ret_val == constants.WMI_JOB_STATUS_STARTED:
            success = self._vmutils.check_job_status(job)
        elif ret_val == 0:
    ...
    def _get_vcpu_total(self):
        ...
        try:
            return self._multiprocessing.cpu_count()
    ...
    def _get_memory_mb_total(self):
        ...
        """
        total_kb = self._conn_cimv2.query("SELECT TotalVisibleMemorySize FROM win32_operatingsystem")[0].TotalVisibleMemorySize
    ...
    def _get_local_gb_total(self):
        ...
        total_kb = self._conn_cimv2.query("SELECT Size FROM win32_logicaldisk WHERE DriveType=3")[0].Size
    ...
    def _get_memory_mb_used(self):
        ...
        """
        total_kb = self._conn_cimv2.query("SELECT FreePhysicalMemory FROM win32_operatingsystem")[0].FreePhysicalMemory
        total_mb = long(total_kb) / 1024
    ...
    def _get_local_gb_used(self):
        ...
        """
        #TODO: This binds to C only right now, need to bind to instance dir
        total_kb = self._conn_cimv2.query("SELECT FreeSpace FROM win32_logicaldisk WHERE DriveType=3")[0].FreeSpace
        total_gb = long(total_kb) / (1024 ** 3)
        return total_gb
    ...
    def _get_hypervisor_version(self):
        ...
        """
        version = self._conn_cimv2.Win32_OperatingSystem()[0].Version.replace('.', '') 
    ...
    def update_available_resource(self, context, host):
        ...
        try:
            service_ref = self._db.service_get_all_compute_by_host(context, host)[0]
        ...
        if not compute_node_ref:
            ...
            self._db.compute_node_create(context, dic)
        else:
            LOG.info(_('Compute_service record updated for %s ') % host)
            self._db.compute_node_update(context, compute_node_ref[0]['id'], dic)
    ...
    def _cache_image(self, fn, target, fname, cow=False, Size=None, *args, **kwargs):
        ...
        def call_if_not_exists(base, fn, *args, **kwargs):
                if not self._os.path.exists(base):
                    fn(target=base, *args, **kwargs)
        
        if not self._os.path.exists(target):
    ...
    def _fetch_image(self, target, context, image_id, user, project, *args, **kwargs):
        """Grab image and optionally attempt to resize it"""
        self._images.fetch(context, image_id, target, user, project)

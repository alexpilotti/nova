...
class HypervVswitchDriver(vif.VIFDriver):
    ...
    #def _create_wvs_vif_port(self, iface_id):
    #    return self._utils.create_switch_port(CONF.vswitch_name, iface_id)

    #def _delete_wvs_vif_port(self, iface_id):
    #    self._utils.delete_switch_port(CONF.vswitch_name, iface_id)

    def plug(self, instance, vif):
        iface_id = vif['id']
        #switch_port = self._create_wvs_vif_port(iface_id)
        vnic = self._utils.create_vnic()
        mac_address = vif['address'].replace(':', '')
        #Connect the new nic to the new port.
        #vnic.Connection = [switch_port]
        vnic.ElementName = iface_id
    ...
    def unplug(self, instance, vif):
        ...
        #iface_id = vif['id']
        #self._delete_wvs_vif_port(iface_id)

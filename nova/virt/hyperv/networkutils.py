...
class NetworkUtils(object):
    ...
    def create_switch_port(self, vswitch_name, switch_port_name):
        ...
        vswitch_path = self._get_vswitch_path_by_name(vswitch_name)
    ...
    def delete_switch_port(self, vswitch_name, switch_port_name):
        ...
        if ret_val != 0:
            ...
            raise vmutils.HyperVException(_('Failed deleting port for %s') % \
                                  vswitch_name)
    ...
    def disconnect_switch_port(self, vswitch_name, switch_port_name):
        ...
        if ret_val != 0:
            ...
            raise vmutils.HyperVException(_('Failed disconnecting '
                'port for %s') % vswitch_name)

    def _get_vswitch_path_by_name(self, vswitch_name):
        q = self._conn.Msvm_VirtualSwitch(ElementName=vswitch_name)
        if not len(q):
            raise vmutils.HyperVException(_('vswitch not found: '
                '%(vswitch_name)s') % locals())            
        return q[0].path_()
